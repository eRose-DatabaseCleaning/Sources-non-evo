#ifndef	__CLASSSTB2_H
#define	__CLASSSTB2_H
//-------------------------------------------------------------------------------------------------

struct tagStrPosDATA {
	long	m_lFilePOS;
	short	m_nStrLEN;
	char   *m_pStr;
...
public :
	...
	bool Create (WORD wDefColCNT, WORD wDefRowCNT)
	{
		...
		m_ppFilePOS = new tagStrPosDATA* [ m_iRowCNT ];
		for (int iR=0; iR<m_iRowCNT; iR++)
			m_ppFilePOS[ iR ] = new tagStrPosDATA[ m_iColCNT ];
	}

	bool AppendRow ()
	{
		tagStrPosDATA **ppFilePOS;
		ppFilePOS = new tagStrPosDATA* [ 1+m_iRowCNT ];

		for (int iR=0; iR<m_iRowCNT; iR++)
			ppFilePOS[iR] = m_ppFilePOS[iR];

		ppFilePOS[m_iRowCNT] = new tagStrPosDATA[ m_iColCNT ];
		m_iRowCNT ++;
	...
	bool AppendColumn ()
	{
		tagStrPosDATA *pFilePOS;

		int iR,iC;
		for (iR=0; iR<m_iRowCNT; iR++) {
			pFilePOS = new tagStrPosDATA[ 1+m_iColCNT ];
